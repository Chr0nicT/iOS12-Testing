...
addr_t find_move_snapshot_to_purgatory(void)
{
    ...
    printf("find_move_snapshot_to_purgatory1: %llu", ref);
    ...
    uint64_t start = bof64(kernel, prelink_base, ref);
    printf("find_move_snapshot_to_purgatory2: %llu", start);
    
    if (!start) {
        return 0;
    }
    printf("find_move_snapshot_to_purgatory3: %llu", start + kerndumpbase);
    return start + kerndumpbase;
}
...
addr_t find_chgproccnt(void)
{
    ...
    uint64_t start = bof64(kernel, xnucore_base, ref);
    printf("chgproccnt2: %llu", start);
    
    if (!start) {
        return 0;
    }
    printf("chgproccnt3: %llu", start + kerndumpbase);
    return start + kerndumpbase;
}
...
addr_t find_kauth_cred_ref(void)
{
    ...
    printf("kauth_cred_ref1: %llu", ref);
    ...
    uint64_t start = bof64(kernel, xnucore_base, ref);
    printf("kauth_cred_ref2: %llu", start);
